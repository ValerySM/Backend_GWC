...
def auth():
    ...
    try:
        ...
        logger.error(f"Error in auth for user {user_id}: {str(e)}")
...
def update():
    ...
    try:
        ...
        if result:
            ...
            logger.info(f"Updated data for user {user_id}: {updates}")
            return jsonify(user_data), 200
        else:
            new_user = {"telegram_id": str(user_id), **updates}
            users.insert_one(new_user)
            logger.info(f"Created new user {user_id} with data: {new_user}")
...
def log():
    data = request.json
    logger.info(f"Client log: {data['message']}")
